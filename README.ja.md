...
- HAR(`.har`)と Fiddler SAZ(`.saz`)のインポート。インポートはアトミックで、ファイルが拒否されても現在のキャプチャは壊れません。
- **オプトインの WebSocket キャプチャ。** ステータスバーの **WS capture** トグルが、DevTools の eval API 経由でページの `WebSocket` コンストラクタをラップし(追加権限なし)、接続を行として記録します。送信フレームは Request の Body ペイン、受信フレームとライフサイクルイベントは Response の Body ペインに入り、他のボディと同様に検索・サニタイズ付きエクスポートの対象です。見えるのはキャプチャ ON 中に作られたソケットだけで、ラッパーが通信を改変することはなく、ページ遷移後は自動で再インストールされます。
