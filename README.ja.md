...
- **漏らさず共有。** すべてのコピーとエクスポートは既定でサニタイズ — HAR に加え、表計算での集計向けにメタデータのみの CSV も出力可能。完全版の出力が既定になることはありません: 行メニューの完全版フォーマットは、渡すものを名前で示す折りたたみ式の `Copy full (unsanitized)` グループにまとまっており、完全版 HAR エクスポートと完全版ボディコピーにはさらに毎回の確認が必要で、選択が記憶されることはありません。
...
| Microsoft Edge | [Edge アドオン](https://microsoftedge.microsoft.com/addons/detail/network-for-devtools/dhmafmhaagefmichhmmkknapalhmlmal) |
...
| ブラウザ | 状態 | 備考 |
|---|---|---|
| Microsoft Edge | プライマリ | 開発・リリース検証の基準環境。[Edge アドオン](https://microsoftedge.microsoft.com/addons/detail/network-for-devtools/dhmafmhaagefmichhmmkknapalhmlmal)で公開中 |
...
- 列ごとのフィルタ: Client start / Server done のローカル時刻レンジピッカー、メソッド複数選択、ドメイン・パスへの繰り返し可能な `contains` / `notcontains` ルール、URL の any / all / exclude ロジック。
...
- **Markdown としてコピー。** 行メニューから、issue にそのまま貼れるサニタイズ済み Markdown ブロック(メソッド・リダクション済み URL・ステータス・Operation・時間)をコピーできます。複数行選択時はコンパクトな Markdown 表も選べます。無加工版は、行メニューの `Copy full (unsanitized)` グループに他 7 つの完全版フォーマットと並んでいます。
...
- 320 px からのレスポンシブ対応。800 px 以下ではリクエスト一覧が詳細パネルの上に積み重なります。
...
<kbd>?</kbd> のアプリ内ダイアログには、800 px 以下で使う縦仕切りのキーも含めた全バインディングが載っています。
...
Network+ はクリップボードと HAR ダウンロードだけを外向きの出口として扱い、安全な形式を既定にしています。完全版出力が既定になることはなく、提供される箇所では常に未サニタイズであることを明示します。確認を伴う箇所では、その確認はその 1 回のアクションにのみ適用され、設定として保存されることはありません。
