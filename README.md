...
2. 通信前の空画面では `Explore sample capture` を選ぶと、外部通信や保存を行わず、成功 API・低速 503・304 キャッシュのローカルサンプル 3 件を表示できる。最初の行が選択されるため、そのまま詳細・Timing・検索・統計を確認できる。サンプル中は実トラフィックとの混在を防ぐためライブ記録を一時停止し、`Clear` でサンプルを削除すると元の記録状態へ戻る
...
- [ ] `Clear` 後は 3 件すべてと詳細・検索・統計が消え、フォーカスを維持したまま通常の空画面とライブ記録へ戻る
