...
- [ ] `Clear` / Undo / サンプル終了後はガイドが閉じた未 reveal 状態へ戻り、試行・移動状態を保存しない
- [ ] 320 / 375 / 414 / 768px の Light / Dark で prompt、reveal、2 つの移動アクション、Close が 24px 以上の操作領域を保ち、文書ルートの横スクロール、文字切れ、画面外ダイアログが発生しない。`Escape` / Close / backdrop はトリガーへ戻り、reveal 後は根拠見出し、移動後は有効な Response タブへフォーカスする
