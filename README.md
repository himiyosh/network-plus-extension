...
- All 133 real-browser regression tests pass under Chrome 151 (`CHROME_BIN=<path> npx jest tests/status-summary-browser.test.js tests/browser-availability-policy.test.js`).
...
- Tabbed inspector: Request (Headers / Body / Query / Cookies / Raw) and Response (Headers / Body / Timing / Cookies / Raw). The Body and Raw views each carry their own keyword search in a toolbar that stays pinned to the top of the pane as it scrolls, with hit highlighting and Enter / Shift+Enter navigation, and response bodies are decoded with the charset their `Content-Type` declares (Shift_JIS, EUC-JP, and friends render correctly). `Body` picks its renderer from the content: a collapsible JSON tree (with a `Tree` / `Text` switch in the pane toolbar for the flat pretty-printed view), HTML in a sandboxed frame with a `Source` switch, and an image painted on a transparency checkerboard — one too small to see is enlarged, with the factor stated beside its real dimensions. Bodies that are not text at all — images, fonts, `.wasm` — are shown as an offset/hex/printable dump rather than decoder mojibake.
