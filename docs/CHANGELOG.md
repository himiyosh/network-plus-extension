...
- Merged the Network+ brand and the support (donation) button into one living toolbar mark: a hand-drawn-style orange tabby — one organic silhouette with tabby stripes, a cream muzzle, sparkly eyes, blushing cheeks, whiskers, and toe-lined paws — rests on top of the “Network+” wordmark itself (no extra toolbar width), blinking and occasionally tilting its head, beside a warm orange coffee cup with a visible coffee surface and faint always-on steam. Roughly every half minute the cat ducks behind the letters and pops back, and now and then it releases a tiny floating heart. Hovering or focusing the mark wakes the cat, strengthens the steam, and slides in a "Support ♥" hint; clicking opens the existing Support dialog. The separate ☕ button is gone, the toolbar is slightly narrower, and all motion stops under `prefers-reduced-motion`.
