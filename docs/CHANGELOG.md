...
- Raised the dark-theme mix percentages for search-hit and manually highlighted rows (8% → 28%/22%) and matched-text marks (25%/20% → 35%/28%) so matching rows are clearly distinguishable on dark surfaces.
- Added a **Matches only** toggle to the search panel: when on, the request list shows only rows that match a search keyword (off keeps every row visible with highlights). The displayed set also drives HAR export, and the setting survives Clear undo.
