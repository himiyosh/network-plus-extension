...
- Merged the Network+ brand and the support (donation) button into one living toolbar mark: a rounder, friendlier take on the dialog's cat peeks over the brand pill — paws on the rim, blinking, cheeks blushing, with an occasional ear flick — beside a coffee cup with faint always-on steam. Roughly every half minute the cat ducks out of sight and pops back, and now and then it releases a tiny floating heart. Hovering or focusing the mark wakes the cat, strengthens the steam, and slides in a "Support ♥" hint; clicking opens the existing Support dialog. The separate ☕ button is gone, the toolbar is slightly narrower, and all motion stops under `prefers-reduced-motion`.
...
- Made search-hit rows unmistakable in the dark theme: row mix percentages raised from 8% to 22%/18%, matched-text marks from 25%/20% to 35%/28%, and every matching row now carries a 3px keyword-colored leading edge and underline ring (70% strength on dark, 45% on light). The `K1`/`K2` match badges moved out of the ID column into the accessibility tree (screen readers still announce them; the tint, ring, and mark colors identify the keyword visually).
- Added a **Matches only** toggle switch directly next to “+ Add keyword” in the search panel: when on, the request list shows only rows that match a search keyword (off keeps every row visible with highlights). The displayed set also drives HAR export, and the setting survives Clear undo. Capture-time notes (“N bodies not searched”, “Showing matches only”) render in a reserved notice area inside the search panel instead of the top bar, so top-bar buttons no longer shift or flicker during live capture.
