...
- Fixed the Presets toolbar button doing nothing: the global outside-click dismisser was closing its popup in the same click that opened it. The popup — with in-place preset saving, applying, and deleting — now opens like the Columns menu.
- Redesigned the Column Filters popup as an accordion: each column is a compact row that expands only when clicked, columns with active rules show an “Active” chip and start expanded, the header keeps a live active-rule count, and a one-line hint explains the interaction. The previous all-columns-at-once wall of controls is gone.
...
- Merged the Network+ brand and the support (donation) button into one living toolbar mark: a small orange cat drawn as one bold organic silhouette — tall pointed ears, big glossy eyes with catchlights, paws resting on the “Network+” wordmark — sized to stay recognizable at its real 15px height (finer details like stripes and whiskers were deliberately dropped as unreadable at that scale), beside a warm orange coffee cup with a visible coffee surface and faint always-on steam. The idle motion is deliberately sparse: blinking, an occasional duck behind the letters, and a rare tiny floating heart. Hovering or focusing the mark wakes the cat, strengthens the steam, and slides in a "Support ♥" hint; clicking opens the existing Support dialog. The separate ☕ button is gone, the toolbar is slightly narrower, and all motion stops under `prefers-reduced-motion`.
