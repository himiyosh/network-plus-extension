...
- `network-plus-extension-1.12.0.zip` was built from the reviewed commit by `npm run extension:package`. It is 206937 bytes and its SHA-256 is `d12593a2d877ade1329aef52e6402f07e83c2a597705f21b7038174011cc6863`. A second local build reproduced the same digest, and archive entries carry fixed timestamps, so `npm run extension:package` at tag `v1.12.0` rebuilds the same bytes. The publishing workflow compares the archive it builds against this digest and fails the release on a mismatch. Public observation of the `v1.12.0` release is a post-merge step and is not claimed here.
