...
- `manifest.json` identifies a Manifest V3 DevTools extension named `Network+ for DevTools`, version `1.11.0`, with one permission (`storage`), packaged 16, 48, and 128 pixel icons, and the extension-page CSP `script-src 'self'; object-src 'self'`.
...
- GitHub release `v1.11.0` and `network-plus-extension-1.11.0.zip` were publicly observable and re-downloaded on 2026-08-21. The ZIP is 152202 bytes and its SHA-256 is `85ea3db4faf0f4a0445420f6e40408a50e9b004a720213bbdb7b2610d36ef91b`. The downloaded file is byte-identical to the local build, CI published the release from the release commit on `main` after checking the archive against this digest, and archive entries carry fixed timestamps, so `npm run extension:package` at tag `v1.11.0` rebuilds the same bytes.
...
- Upload `network-plus-extension-1.11.0.zip` obtained from the trusted release route into the existing product's package section, and compare the complete 64-character SHA-256 recorded above against the downloaded file before submitting.
...
- Certification notes for an update should say what changed. The `## v1.11.0` section of `docs/CHANGELOG.md` is the reviewed source for that text.
...
1. Use the exact ZIP uploaded for certification. For repository-side reproduction, run `npm run extension:package`, extract `dist/network-plus-extension-1.11.0.zip` into a new folder, open `edge://extensions/`, enable Developer mode, choose `Load unpacked`, and select the extracted folder. A ZIP file itself is not selected by `Load unpacked`.
