...
- GitHub release `v1.10.0` and `network-plus-extension-1.10.0.zip` were publicly observable and re-downloaded on 2026-08-21. The ZIP is 152096 bytes and its SHA-256 is `11e308be9a4982f0afc608138fdebad6029f650ac87b969b0b3c00a92d6cd25d`. The downloaded file is byte-identical to the local build, CI published the release from the release commit on `main` after checking the archive against this digest, and archive entries carry fixed timestamps, so `npm run extension:package` at tag `v1.10.0` rebuilds the same bytes.
