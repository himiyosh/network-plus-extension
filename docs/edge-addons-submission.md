...
- `network-plus-extension-1.11.0.zip` was built from the reviewed commit by `npm run extension:package`. It is 152202 bytes and its SHA-256 is `85ea3db4faf0f4a0445420f6e40408a50e9b004a720213bbdb7b2610d36ef91b`. A second local build reproduced the same digest, and archive entries carry fixed timestamps, so `npm run extension:package` at tag `v1.11.0` rebuilds the same bytes. The publishing workflow compares the archive it builds against this digest and fails the release on a mismatch. Public observation of the `v1.11.0` release is a post-merge step and is not claimed here.
