...
describe('outbound sensitive-data policy', () => {
  ...
  test('reads HAR creator versions from the extension runtime with a Node-test fallback', () => {
    ...
    expect(np.getExtensionVersion({ getManifest: () => { throw new Error('runtime-failed'); } })).toBe('1.12.0');
    expect(np.getExtensionVersion(null)).toBe('1.12.0');
  });
  ...
  test('uses the current extension version for full and sanitized HAR creators', () => {
    ...
    expect(fullHar.log.creator.version).toBe('1.12.0');
    expect(np.sanitizeHar(fullHar).log.creator.version).toBe('1.12.0');
  });
});
